...
//构造函数
T_Block::T_Block(int x, int y, int att) {
	this->b_x = x;
	this->b_y = y;
	this->attitude = att;
}
...
//添加入地图
void T_Block::add_map(vector<vector<int>> &arr) {
...
//边界判断
bool T_Block::Boundary_judgment(vector<vector<int>> &arr) {
...
//移动边界判断
bool T_Block::Move_judgment(vector<vector<int>> &arr, int state) {
	switch (state) {
	case 1://左移
	...
	case 3://逆时针旋转
		if (this->attitude - 1 == 1) {
			if ((arr[this->b_y - 1][this->b_x] == 0) && (arr[this->b_y][this->b_x - 2] == 0) && (arr[this->b_y][this->b_x + 2] == 0)) {
				return true;
			}
			else {
				return false;
			...
			if ((arr[this->b_y][this->b_x + 2] == 0) && (arr[this->b_y - 1][this->b_x] == 0) && (arr[this->b_y + 1][this->b_x] == 0)) {
				return true;
			}
			else {
				return false;

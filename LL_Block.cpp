...
//构造函数
LL_Block::LL_Block(int x, int y, int att) {
	this->b_x = x;
	this->b_y = y;
	this->attitude = att;
}
...
//添加入地图
void LL_Block::add_map(vector<vector<int>> &arr) {
...
//边界判断
bool LL_Block::Boundary_judgment(vector<vector<int>> &arr) {
...
//移动边界判断
bool LL_Block::Move_judgment(vector<vector<int>> &arr, int state) {

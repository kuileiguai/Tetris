...
//抽象一个逆时针旋转
void High_Block::anti_clockwise() {
	if (this->attitude == 2) {
		this->attitude = 1;
	}
	else {
...
High_Block::High_Block(int x, int y, int att) {
	this->b_x = x;
	this->b_y = y;
	this->attitude = att;
}
...
//添加入地图
void High_Block::add_map(vector<vector<int>> &arr) {
...
//边界判断
bool High_Block::Boundary_judgment(vector<vector<int>> &arr) {
...
//移动边界判断
bool High_Block::Move_judgment(vector<vector<int>> &arr, int state) {
	switch (state) {
	case 1://左移
